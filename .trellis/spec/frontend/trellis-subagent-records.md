...
- `AgentEventConnection` rejects discarded EventSource callbacks. Reconnect marks overlays stale and retains the union of historical calls plus live assistant calls already authorized by the same current view; session/branch reset still clears that union. Upstream's selected-session replay re-sends the latest in-memory `tool_execution_start`/`tool_execution_update` before its assistant snapshot. Already-authorized Trellis updates are untrusted partial last-reported evidence only (never proof of branch ownership or process liveness). An unknown replay is held in a 32-event buffer until the assistant snapshot grants call ownership, and is dropped if session, leaf, view generation, settlement, or the snapshot gate does not match. A replayed start/update never grants ownership itself or moves the viewed branch; later `tool_execution_end`/canonical message supersedes partial evidence by rank. Refresh is skipped while history is in flight or parent/prompt/SDK is active; settlement reconciles later. Historical settlement refreshes that context rather than jumping to head.
...
- `hooks/useAgentSession.trellis.test.mjs` and `e2e/subagents.mjs`: delayed A→B→A success/404/finally, X→Y→X context and stale navigation, pagination races, natural StrictMode stream open, lease recovery, already-authorized replay, replay-before-snapshot buffering, final reconciliation, historical settlement, old owner cleanup and real-ID promotion. Source wiring assertions supplement reducer/browser behavior, not replace it.
