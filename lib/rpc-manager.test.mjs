...
test("closeAsk delivers answers fire-and-forget so ask_submit answers promptly", async () => {
  // With the agent idle, sendCustomMessage({ triggerTurn: true }) runs the
  // full agent prompt and only resolves when that turn finishes. Awaiting it
  // in closeAsk would hold the ask_submit response open for the whole run,
  // leaving the browser card stuck in its "submitted" state. The send must be
  // detached (void + catch) and the response returned immediately.
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const closeAskSource = source.slice(
    source.indexOf("private async closeAsk"),
    source.indexOf("voidOpenAskForUserMessage"),
  );
  assert.match(closeAskSource, /void this\.inner\.sendCustomMessage\(/);
  assert.match(closeAskSource, /\.catch\(\(error\) => \{/);
  assert.doesNotMatch(closeAskSource, /await this\.inner\.sendCustomMessage/);
  assert.match(closeAskSource, /return \{ result: "closed", outcome, pendingAsk: this\.pendingAsk \};/);
});
...
test("get_state reports an unknown context usage instead of failing the whole response", async () => {
  // pi's getContextUsage() walks the branch and reads `usage.totalTokens` off
  // the newest assistant entry, which a hand-written or legacy v3 session file
  // can omit — measured against the e2e compacted fixture, where it throws
  // `Cannot read properties of undefined (reading 'totalTokens')`. This command
  // backs GET /api/agent/[id] on session open and the
  // GET /api/sessions/[id]/state poll, so an escaped throw would answer 500 and
  // render the session unopenable. Only the derived metric may go missing.
  const root = await mkdtemp(join(tmpdir(), "pi-web-context-usage-"));
  const sessionDir = join(root, "sessions");
  await mkdir(sessionDir);
  const manager = SessionManager.create(root, sessionDir);
  manager.appendMessage({ role: "user", content: "no usage anywhere", timestamp: Date.now() });
  const wrapper = new AgentSessionWrapper({
    sessionId: manager.getSessionId(),
  ...
  try {
    ...
    assert.equal(state.sessionId, manager.getSessionId());
  } finally {
    wrapper.destroy();
