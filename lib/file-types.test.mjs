...
test("buckets files for the turn artifact card", async () => {
  const { getFileCategory } = await loadSubject();

  assert.equal(getFileCategory("/repo/tasks/prd.md"), "document");
  assert.equal(getFileCategory("/repo/src/app.tsx"), "code");
  assert.equal(getFileCategory("/repo/assets/logo.PNG"), "image");
  assert.equal(getFileCategory("/repo/data/rows.jsonl"), "data");
  assert.equal(getFileCategory("/repo/.env"), "config");
  assert.equal(getFileCategory("/repo/Makefile"), "other");
  assert.equal(getFileCategory("/repo/LICENSE"), "other");
});
