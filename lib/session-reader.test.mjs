...
test("concurrent listings share one scan; force bypasses the fresh server cache", async (t) => {
  ...
  // Both callers resolve to the same array instance: the second call joined
  // the first call's in-flight scan (single-flight) instead of rescanning.
  assert.equal(r1, r2);
  assert.equal(r1.length, 1);

  // force=1 invalidates the fresh 30s cache and starts a brand-new scan.
  const forced = await listAllSessions({ force: true });
  assert.notEqual(forced, r1);
