...
// Isolate scan-based tests (listAllSessions) behind a throwaway agent dir so
// they exercise the real incremental scan without touching ~/.pi/agent.
const scanTestAgentDir = mkdtempSync(join(tmpdir(), "pi-web-session-scan-"));
process.env.PI_CODING_AGENT_DIR = scanTestAgentDir;
const scanTestSessionsDir = join(scanTestAgentDir, "sessions");
mkdirSync(scanTestSessionsDir, { recursive: true });
test.after(() => rmSync(scanTestAgentDir, { recursive: true, force: true }));
...
test("round-trips a stall-abort custom message so a reload keeps the reason", () => {
  const details = {
    toolName: "bash",
    timeoutMs: 900_000,
  ...
  const entries = [
    userEntry("u1", null, "start"),
    {
      ...
      content: "Turn aborted automatically: no output from bash for 16 min (1h 35m running).",
      display: true,
      details,
    },
  ];
  ...
  assert.deepEqual(context.entryIds, ["u1", "c-stall"]);
  assert.equal(context.messages[1].role, "custom");
  assert.equal(context.messages[1].customType, "pi-web.stall.abort");
  assert.equal(context.messages[1].display, true);
  assert.match(context.messages[1].content, /no output from bash for 16 min/);
  assert.deepEqual(context.messages[1].details, details);
});
...
test("session listing reads subagent relations and terminal status without reopening full session files", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-relation-prefix-"));
  const projectDir = join(agentDir, "sessions", "project");
  mkdirSync(projectDir, { recursive: true });
  const filePath = join(projectDir, "2026-01-01T00-00-00-000Z_child.jsonl");
  ...
  assert.equal(sessions[0]?.id, childId);
...
function writeScanSession(fileName, id) {
  const lines = [
    JSON.stringify({ type: "session", version: 3, id, timestamp: "2026-01-01T00:00:00.000Z", cwd: "/tmp/non-git-project" }),
    JSON.stringify({ type: "message", id: "m1", parentId: null, message: { role: "user", content: "hello" }, timestamp: "2026-01-01T00:01:00.000Z" }),
  ];
  // pi stores sessions under <sessionsDir>/<encoded-cwd>/<file>.jsonl
  const cwdDir = join(scanTestSessionsDir, "--tmp--");
  mkdirSync(cwdDir, { recursive: true });
  writeFileSync(join(cwdDir, fileName), lines.join("\n") + "\n");
}
...
test("readSessionById returns a targeted summary without a catalogue scan", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-read-by-id-"));
  const sessionId = "target-read-session";
  const projectDir = join(agentDir, "sessions", "project");
  const filePath = join(projectDir, `2026-01-01T00-00-00-000Z_${sessionId}.jsonl`);
  mkdirSync(projectDir, { recursive: true });
  writeFileSync(filePath, [
    ...
    JSON.stringify(userEntry("u1", null, "first user message", "2026-01-01T00:01:00.000Z")),
    JSON.stringify(assistantEntry("a1", "u1", "reply", "2026-01-01T00:02:00.000Z")),
  ].join("\n") + "\n");
  ...
  assert.equal(session.firstMessage, "first user message");
  assert.equal(session.modified, "2026-01-01T00:02:00.000Z");
  assert.equal(listAllCalls, 0);
  assert.equal(globalThis.__piSessionListCache, undefined, "targeted lookup must not run the catalogue scan");

  assert.equal(await readSessionById("does-not-exist"), null);
  assert.equal(listAllCalls, 0, "incremental scanning must never call SessionManager.listAll");
});
...
test("falls back to the catalogue when a targeted lookup finds duplicate IDs", async (t) => {
  ...
  // The catalogue scan resolves the id to one of the duplicate files.
  const resolved = await resolveSessionPath(sessionId);
  assert.ok(resolved);
});

test("a scan invalidated in flight retries before returning to its caller", async (t) => {
  writeScanSession("2026-01-01T00-00-00-000Z_a.jsonl", "a");
  resetSessionListState();
  t.after(() => {
    resetSessionListState();
  });

  const listing = listAllSessions();
  invalidateSessionListCache(); // invalidate while the first scan is in flight
  const result = await listing;
  // The stale scan result is discarded; the caller receives a fresh scan that
  // completed after the invalidation.
  assert.equal(result.length, 1);
  assert.ok(globalThis.__piSessionListCache);
});

test("forced session listing bypasses the fresh server cache", async (t) => {
  writeScanSession("2026-01-01T00-00-00-000Z_force.jsonl", "force-session");
  resetSessionListState();
  t.after(() => {
    resetSessionListState();
  });
  ...
  assert.ok(first.some((s) => s.id === "force-session"));
  const version = getSessionListVersion();

  // Cached reads return the same snapshot and must not bump the version again.
  const cached = await listAllSessions();
  ...
  // A session written within the 30s TTL is invisible to the cached read…
  writeScanSession("2026-01-01T00-00-00-001Z_force2.jsonl", "force-session-2");
  assert.ok(!(await listAllSessions()).some((s) => s.id === "force-session-2"));

  // …but a force refresh bypasses the cache and re-scans.
  const forced = await listAllSessions({ force: true });
  assert.ok(forced.some((s) => s.id === "force-session-2"));
