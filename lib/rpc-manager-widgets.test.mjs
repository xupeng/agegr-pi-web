...
const testAgentDir = mkdtempSync(join(tmpdir(), "pi-web-rpc-widgets-"));
...
} = await jiti.import("./extension-ui-settings.ts");
...
test("suppresses default powerline widgets before invoking factories", async () => {
  const { context, events, wrapper } = createContext();
  let factoryCalls = 0;

  context.setWidget("powerline-top", () => {
    factoryCalls += 1;
    return { render: () => ["hidden"] };
  });

  assert.equal(factoryCalls, 0);
  assert.deepEqual(widgetEvents(events), []);
  assert.deepEqual(await snapshot(wrapper), []);
  wrapper.destroy();
});

test("applies configured widget and status rules and clears stale state", async () => {
  writeExtensionUiVisibilitySettings({ hiddenWidgetKeys: [], hiddenStatusKeys: [] });
  const { context, events, wrapper } = createContext();
  context.setWidget("task-panel", ["visible"]);
  context.setStatus("task-status", "visible");
  assert.equal((await snapshot(wrapper)).length, 1);

  writeExtensionUiVisibilitySettings({
    hiddenWidgetKeys: ["task-*"],
    hiddenStatusKeys: ["task-status"],
  });
  context.setWidget("task-panel", ["replacement"]);
  context.setStatus("task-status", "replacement");

  assert.equal(widgetEvents(events).at(-1).widgetLines, undefined);
  assert.equal(statusEvents(events).at(-1).statusText, undefined);
  ...
  wrapper.destroy();
  writeExtensionUiVisibilitySettings({ hiddenWidgetKeys: ["powerline-*"], hiddenStatusKeys: [] });
});
