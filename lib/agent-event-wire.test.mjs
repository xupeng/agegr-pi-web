...
test("preserves structured Trellis subagent details in partial and end events", () => {
  const details = {
    kind: "trellis-subagent-progress",
    agent: "trellis-implement",
    mode: "parallel",
    final: false,
    runs: [{ id: "run-1", status: "running", finalText: "", textTail: "", thinkingTail: "", stderrTail: "", tools: [] }],
  };
  const update = toClientAgentEvent({
    type: "tool_execution_update",
    toolCallId: "call-1",
    toolName: "trellis_subagent",
    partialResult: { content: [{ type: "text", text: "subagent running" }], details },
  });
  assert.deepEqual(update.partialResult.details, details);

  const end = toClientAgentEvent({
    type: "tool_execution_end",
    toolCallId: "call-1",
    toolName: "trellis_subagent",
    result: { content: [{ type: "text", text: "done" }], details: { ...details, final: true } },
    isError: false,
  });
  assert.equal(end.result.details.kind, "trellis-subagent-progress");
  assert.equal(end.result.details.final, true);
});
