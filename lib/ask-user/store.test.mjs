...
test("malformed runtime questions stay errors and do not replace an open ask", () => {
  const store = makeStore();
  const { ask } = openAsk(store);
  const malformed = [
    undefined,
    null,
    { id: "q9", question: "not a list" },
    [null],
    [{ id: 1, question: "bad id", options: [] }],
    [{ id: "q9", question: "bad options", options: "nope" }],
    [{ id: "q9", question: "bad option", options: [null] }],
    [{ id: "q9", question: 12, options: [] }],
  ];
  for (const questions of malformed) {
    assert.throws(() => store.open({ sessionId: "s1", questions }), PendingAskValidationError);
  }
  assert.equal(store.pendingAsk("s1").askId, ask.askId);
});

test("malformed runtime submissions stay errors and leave the ask open", () => {
  const store = makeStore();
  const { ask } = openAsk(store);
  assert.throws(() => store.submit("s1", ask.askId, undefined), /Answers must be a list/);
  assert.throws(() => store.submit("s1", ask.askId, { answers: null }), /Answers must be a list/);
  assert.throws(() => store.submit("s1", ask.askId, { answers: [null] }), /Answer must be an object/);
  assert.throws(
    () => store.submit("s1", ask.askId, { answers: [{ id: "q1", values: "pg" }] }),
    /must list values/,
  );
  assert.throws(
    () => store.submit("s1", ask.askId, { answers: [{ id: "q1", values: [1] }] }),
    /has no option/,
  );
  assert.throws(
    () => store.submit("s1", ask.askId, { answers: [{ id: "q1", values: ["pg"] }], supplement: 12 }),
    /Supplement must be text/,
  );
  assert.equal(store.pendingAsk("s1").askId, ask.askId);
});
