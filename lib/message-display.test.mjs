...
// `message-display.ts` now has runtime imports (shared i18n copy), so it is
// loaded through jiti like other lib modules instead of bare `import()`.
...
test("renderStallAbortText builds the persisted English summary from stall details", async () => {
  const { renderStallAbortText, STALL_ABORT_CUSTOM_TYPE } = await loadSubject();
  assert.equal(STALL_ABORT_CUSTOM_TYPE, "pi-web.stall.abort");

  const withTool = renderStallAbortText({
    toolName: "bash",
    timeoutMs: 15 * 60_000,
    timeoutSource: "env",
    toolOverride: false,
  ...
  assert.match(withTool, /no output from bash for 16 min/);
  assert.match(withTool, /1h 35m/);

  const noTool = renderStallAbortText({ silentMs: 60_000, elapsedMs: 90_000 });
  assert.match(noTool, /no output for 1 min/);
  assert.match(noTool, /1m 30s/);

  // Malformed payloads must not produce NaN or throw.
  for (const value of [null, "stall", { silentMs: Number.NaN }, { silentMs: -5 }]) {
    const text = renderStallAbortText(value);
    assert.equal(text.includes("NaN"), false, `payload ${JSON.stringify(value)} leaked NaN`);
    assert.match(text, /no output for 1 min/);
  }
});
