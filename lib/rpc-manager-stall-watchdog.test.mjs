...
test("AC1: aborts a stalled turn and reports the in-flight tool and elapsed time", async (t) => {
  ...
  // AC7: the same stall is persisted as a session custom message so the reason
  // survives a reload; `triggerTurn: false` keeps it from starting a new turn.
  assert.equal(inner.customMessages.length, 1, "the stall reason must be written exactly once");
  const persisted = inner.customMessages[0];
  assert.equal(persisted.message.customType, "pi-web.stall.abort");
  assert.equal(persisted.message.display, true);
  assert.deepEqual(persisted.options, { triggerTurn: false });
  assert.equal(persisted.message.details.toolName, "read");
  assert.equal(persisted.message.details.silentMs, 60_000);
  assert.equal(persisted.message.details.timeoutMs, 60_000);
  assert.equal(persisted.message.details.timeoutSource, "env");
  assert.equal(persisted.message.details.toolOverride, false);
  assert.equal(persisted.message.details.elapsedMs, 60_000);
  assert.equal(persisted.message.details.toolElapsedMs, 60_000);
  assert.equal(typeof persisted.message.content, "string");
  assert.match(persisted.message.content, /read/);
});

test("AC7: a failed transcript write is logged and does not block the abort", async (t) => {
  const AgentSessionWrapper = await importWrapper("60000");
  t.mock.timers.enable({ apis: ["setTimeout", "Date"] });
  const errorCalls = [];
  ...
  const inner = makeControllableInner();
  inner.sendCustomMessage = () => Promise.reject(new Error("disk full"));
  const wrapper = new AgentSessionWrapper(inner);
  t.after(() => wrapper.destroy());
  const events = [];
  wrapper.onEvent((event) => events.push(event));
  ...
  assert.equal(inner.abortCalls, 1, "the abort must run even when the write fails");
  assert.equal(countStallEvents(events), 1);
  assert.ok(
    errorCalls.some((args) => String(args[0]).includes("failed to record stalled session abort")),
