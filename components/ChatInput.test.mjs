...
const { ChatInput, replaceLinksWithMarkdown, ModelErrorBanner, ModelScopeWarningBanner, canClearBuiltinCommandInput, canRestoreUserMessage, canRunBuiltinSlashCommandWhileStreaming, compressImageFile, filterModelOptions, getUpwardMenuMaxHeight, getUserMessageText, getUserMessageDraftImages, isExactSlashCommand, modelSupportsImageInput, shouldCompressImageFile } = await jiti.import("./ChatInput.tsx");
...
test("detects image input support from exact model metadata", () => {
  ...
  assert.equal(modelSupportsImageInput({ provider: "ollama", modelId: "unknown" }, [{ provider: "ollama", id: "unknown" }]), false);
  assert.equal(modelSupportsImageInput({ provider: "ollama", modelId: "missing" }, models), false);
...
test("routes image attachments by current model capability", async () => {
  ...
  // Image processing runs inside compressImageFile, which is called after the
  // pending placeholder is published (base64 decoding happens only later).
  const readAt = source.indexOf("compressImageFile(file)", pendingAt);

  assert.ok(pendingAt > processStart, "shows the inline-processing placeholder immediately");
  assert.ok(readAt > pendingAt, "shows the placeholder before image processing");
...
test("renders the upstream model error", () => {
  const html = renderToStaticMarkup(
    React.createElement(I18nProvider, null, React.createElement(ModelErrorBanner, {
...
test("renders enabledModels scope warnings", () => {
  const html = renderToStaticMarkup(
    React.createElement(I18nProvider, null, React.createElement(ModelScopeWarningBanner, {
  ...
  assert.equal(renderToStaticMarkup(React.createElement(I18nProvider, null, React.createElement(ModelScopeWarningBanner, { warnings: [] }))), "");
});
